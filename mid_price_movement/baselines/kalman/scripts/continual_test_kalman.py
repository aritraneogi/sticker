"""
continual_test_kalman.py  (Kalman Filter MPM Prequential Test)
==========================================================================
Strict, leak-free, prequential (test-then-train) continual streaming test
for Mid Price Movement (MPM) prediction using a 2D Kalman Filter (State: Mid Price + Velocity).
...
Storage:
  Results: evaluation-test-mpm/results_kalman/
  Weights: evaluation-test-mpm/results_kalman/weights/
  Graphs:  evaluation-test-mpm/graphs_kalman/
"""
...
RESULTS_DIR = SCRIPT_DIR / "results_kalman"
...
def parse_args():
    ...
    p.add_argument("--q-var",       type=float, default=1e-5)
    p.add_argument("--r-var",       type=float, default=1e-3)
...
def post_analysis(pred_path: Path, horizons: list[int], split_tick: int) -> dict:
    print("[POST-ANALYSIS Kalman] loading predictions.parquet ...")
...
class StreamingKalmanFilter:
    """2D Kinematic Kalman Filter for Mid Price & Velocity Estimation"""
    def __init__(self, q_var=1e-5, r_var=1e-3):
        self.x = np.zeros((2, 1), dtype=np.float64) # [mid, velocity]^T
        ...
        self.Q = np.eye(2, dtype=np.float64) * q_var
    ...
    def update(self, mid_val: float) -> int:
        ...
        # Predict direction BEFORE measurement correction (prequential)
        velocity_pred = float(x_prior[1, 0])
        pred_sign = 1 if velocity_pred >= 0 else -1

        # Measurement update
        z = np.array([[mid_val]], dtype=np.float64)
        y = z - (self.H @ x_prior) # Innovation
...
def main():
    ...
    print(f"[CONFIG Kalman] coin={coin}  dates={len(all_dates)}  hours={hours[0]}-{hours[-1]}")

    split_tick_est = args.split_tick if args.split_tick else (int(args.max_ticks * SPLIT_RATIO) if args.max_ticks else None)

    kf = StreamingKalmanFilter(q_var=args.q_var, r_var=args.r_var)
    ...
    print("[START Kalman] streaming mid price movements ...")
            ...
            if feat_vec is not None:
                ...
                if len(buf_preds) == BS:
                    ...
                    if tick_idx - last_snap >= SNAPSHOT_EVERY:
                        ...
                        print(f"[t={tick_idx:>12,}] Kalman {date_str} | "
    ...
    elapsed = time.time() - t0
    print(f"\n[DONE Kalman] total_ticks={tick_idx:,}  elapsed={elapsed:.1f}s  tps={tick_idx/elapsed:.0f}")
    ...
    summary = {
        "model": "KalmanFilter", "coin": coin, "total_ticks": tick_idx,
    ...
    print(f"\n{'='*85}")
    print(f"  FINAL SUMMARY (Kalman Filter)  |  {coin}  |  {tick_idx:,} ticks  |  {elapsed:.0f}s  ({tick_idx/elapsed:.0f} tps)")
    print(f"{'='*85}")
    for h in HORIZONS:
