"""
continual_test_deeplob.py  (DeepLOB MPM Prequential Continual Test)
===================================================================
Strict, leak-free, prequential (test-then-train) continual streaming test
for Mid Price Movement (MPM) prediction using the DeepLOB PyTorch architecture.
Ref: Zhang et al. (2019) IEEE Transactions on Signal Processing.
...
Storage:
  Results: evaluation-test-mpm/results_deeplob/
  Weights: evaluation-test-mpm/results_deeplob/weights/
  Graphs:  evaluation-test-mpm/graphs_deeplob/
"""
...
RESULTS_DIR = SCRIPT_DIR / "results_deeplob"
WEIGHTS_DIR = RESULTS_DIR / "weights"

sys.path.insert(0, str(ROOT_DIR))
sys.path.insert(0, str(SCRIPT_DIR))
from features import FeatureEngine, FEATURE_NAMES
from model.deeplob_model import DeepLOB
...
def post_analysis(pred_path: Path, horizons: list[int], split_tick: int) -> dict:
    print("[POST-ANALYSIS DeepLOB] loading predictions.parquet ...", flush=True)
...
def main():
    ...
    print(f"[CONFIG DeepLOB] coin={coin}  dates={len(all_dates)}  hours={hours[0]}-{hours[-1]}  threads={args.threads}  device={device}", flush=True)

    split_tick_est = args.split_tick if args.split_tick else (int(args.max_ticks * SPLIT_RATIO) if args.max_ticks else None)

    model = DeepLOB(in_features=56, seq_len=SEQ_LEN, lstm_hidden=64).to(device)
    ...
    print("[START DeepLOB] streaming mid price movements ...", flush=True)
            ...
            if feat_vec is not None:
                ...
                if len(buf_seqs) == BS:
                    ...
                    if tick_idx - last_snap >= SNAPSHOT_EVERY:
                        ...
                        print(f"[t={tick_idx:>12,}] DeepLOB {date_str} | loss={ls} | "
    ...
    elapsed = time.time() - t0
    print(f"\n[DONE DeepLOB] total_ticks={tick_idx:,}  elapsed={elapsed:.1f}s  tps={tick_idx/elapsed:.0f}", flush=True)
    ...
    summary = {
        "model": "DeepLOB", "coin": coin, "total_ticks": tick_idx,
    ...
    print(f"\n{'='*85}", flush=True)
    print(f"  FINAL SUMMARY (DeepLOB)  |  {coin}  |  {tick_idx:,} ticks  |  {elapsed:.0f}s  ({tick_idx/elapsed:.0f} tps)", flush=True)
    ...
    print(f"\n[OUTPUT DeepLOB] {RESULTS_DIR}", flush=True)
