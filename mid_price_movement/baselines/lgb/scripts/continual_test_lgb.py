"""
continual_test_lgb.py  (LightGBM / HistGradientBoosting MPM Prequential Test)
================================================================================
Strict, leak-free, prequential (test-then-train) continual streaming test
for Mid Price Movement (MPM) prediction using LightGBM/HistGradientBoostingClassifier.
...
Storage:
  Results: evaluation-test-mpm/results_lgb/
  Weights: evaluation-test-mpm/results_lgb/weights/
  Graphs:  evaluation-test-mpm/graphs_lgb/
"""
...
RESULTS_DIR = SCRIPT_DIR / "results_lgb"
...
def post_analysis(pred_path: Path, horizons: list[int], split_tick: int) -> dict:
    print("[POST-ANALYSIS LightGBM] loading predictions.parquet ...")
...
def main():
    ...
    print(f"[CONFIG LightGBM] coin={coin}  dates={len(all_dates)}  hours={hours[0]}-{hours[-1]}")
    print(f"[CONFIG LightGBM] max_iter={args.max_iter}  lr={args.lr}  batch={BS}")
    ...
    # Rolling window buffer for GBDT training (100k rolling window)
    window_X = deque(maxlen=100_000)
    window_y = deque(maxlen=100_000)
    ...
    print("[START LightGBM] streaming mid price movements ...")

    def process_batch(b_tick_start, b_f, b_mids, b_l, b_m):
        nonlocal is_fitted, model

        t_inf = time.perf_counter()
        if is_fitted:
            preds = model.predict(b_f)
        ...
        # Accumulate non-zero labels into rolling window
        valid_mask = b_l != 0
        if valid_mask.any():
            for f_vec, l_val in zip(b_f[valid_mask], b_l[valid_mask]):
                window_X.append(f_vec)
                window_y.append(int(l_val))
        ...
        for px, sz, side, ts_ns in iter_trades_fast(date_str, hours, coin, trades_dir):
            ...
            if feat_vec is not None:
                ...
                if len(buf_feats) == BS:
                    ...
                    # Periodic incremental GBDT refit
                    if len(window_y) >= 1_000 and (tick_idx - last_train >= RETRAIN_EVERY or not is_fitted):
                        X_tr = np.array(window_X)
                        y_tr = np.array(window_y)
                    ...
                    if tick_idx - last_snap >= SNAPSHOT_EVERY:
                        ...
                        snap = {
                            ...
                            "window_size": len(window_y), "tps": round(tick_idx / elapsed, 0),
                            "lat_ms": round(lat_ms,3),
                        }
                        for h in HORIZONS:
                        ...
                        print(f"[t={tick_idx:>12,}] LightGBM {date_str} | "
    ...
    if buf_feats:
        ...
        b_mids = np.array(buf_mids, dtype=np.float64)
        b_l = np.array(buf_labels, dtype=np.int8)
        process_batch(tick_idx - len(buf_feats), b_f, b_mids, b_l, buf_meta)

    elapsed = time.time() - t0
    print(f"\n[DONE LightGBM] total_ticks={tick_idx:,}  elapsed={elapsed:.1f}s  tps={tick_idx/elapsed:.0f}")
    ...
    summary = {
        "model": "LightGBM_HistGradientBoosting", "coin": coin, "total_ticks": tick_idx,
        "split_tick_70_exact":  true_split_tick,
        "n_features": _N_FEATURES, "feature_names": FEATURE_NAMES,
        "max_iter": args.max_iter, "learning_rate": args.lr, "batch_size": BS,
    ...
    print(f"\n{'='*85}")
    print(f"  FINAL SUMMARY (LightGBM)  |  {coin}  |  {tick_idx:,} ticks  |  {elapsed:.0f}s  ({tick_idx/elapsed:.0f} tps)")
    print(f"{'='*85}")
    for h in HORIZONS:
