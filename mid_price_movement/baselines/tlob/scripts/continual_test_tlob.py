"""
continual_test_tlob.py  (TLOB MPM Prequential Continual Test)
============================================================
Strict, leak-free, prequential (test-then-train) continual streaming test
for Mid Price Movement (MPM) prediction using the Transformer LOB (TLOB) PyTorch architecture.
Ref: Wallbridge (2020) "Transformers for Limit Order Books", arXiv:2003.00130.
...
Storage:
  Results: evaluation-test-mpm/results_tlob/
  Weights: evaluation-test-mpm/results_tlob/weights/
  Graphs:  evaluation-test-mpm/graphs_tlob/
"""
...
RESULTS_DIR = SCRIPT_DIR / "results_tlob"
WEIGHTS_DIR = RESULTS_DIR / "weights"

sys.path.insert(0, str(ROOT_DIR))
sys.path.insert(0, str(SCRIPT_DIR))
from features import FeatureEngine, FEATURE_NAMES
from model.tlob_model import TLOB
...
def post_analysis(pred_path: Path, horizons: list[int], split_tick: int) -> dict:
    print("[POST-ANALYSIS TLOB] loading predictions.parquet ...", flush=True)
...
def main():
    ...
    if not all_dates:
        print(f"[ERROR] No extracted trade data in {trades_dir}", flush=True)
        sys.exit(1)
    ...
    print(f"[CONFIG TLOB] coin={coin}  dates={len(all_dates)}  hours={hours[0]}-{hours[-1]}  threads={args.threads}  device={device}  batch_size={BS}", flush=True)

    split_tick_est = args.split_tick if args.split_tick else (int(args.max_ticks * SPLIT_RATIO) if args.max_ticks else None)

    model = TLOB(in_features=56, seq_len=SEQ_LEN, d_model=64, n_heads=4, n_layers=2).to(device)
    ...
    buf_labels = []
    buf_meta   = []
    prev_seqs  : Optional[np.ndarray] = None
    ...
    print("[START TLOB] streaming mid price movements ...", flush=True)

    def process_batch(b_tick_start, b_s, b_mids, b_l, b_m, p_s, p_l):
        nonlocal n_trained
        loss_val = None

        x_tensor = torch.from_numpy(b_s).float().to(device)

        t_inf = time.perf_counter()
        model.eval()
        with torch.inference_mode():
            logits = model(x_tensor)
            preds  = torch.where(logits >= 0, 1, -1).cpu().numpy()
        lat_ms = (time.perf_counter() - t_inf) * 1e3

        hq.record_batch(b_tick_start, b_mids, preds)
        hq.resolve_batch(b_tick_start, b_mids)

        # Train step on previous batch
        if p_s is not None and p_l is not None:
            valid_mask = p_l != 0
            if valid_mask.any():
                p_x = torch.from_numpy(p_s[valid_mask]).float().to(device)
                p_y = torch.from_numpy((p_l[valid_mask] > 0).astype(np.float32)).to(device)

                model.train()
                optimizer.zero_grad(set_to_none=True)
                out_logits = model(p_x)
                loss = criterion(out_logits, p_y)
                loss.backward()
        ...
        for px, sz, side, ts_ns in iter_trades_fast(date_str, hours, coin, trades_dir):
            ...
            if feat_vec is not None:
                ...
                if len(seq_buffer) < SEQ_LEN:
                    pad = np.tile(seq_buffer[0], (SEQ_LEN - len(seq_buffer), 1))
                    full_seq = np.vstack([pad, np.array(seq_buffer)])
                else:
                    full_seq = np.array(seq_buffer)

                buf_seqs.append(full_seq)
                buf_mids.append(mid)
                buf_labels.append(mid_label)
                buf_meta.append((tick_idx, date_str, ts_ns, px, sz, side))

                if len(buf_seqs) == BS:
                    b_s = np.stack(buf_seqs)
                    b_mids = np.array(buf_mids, dtype=np.float64)
                    b_l = np.array(buf_labels, dtype=np.int8)
                    b_m = buf_meta.copy()
                    b_tick_start = tick_idx - BS + 1

                    lat_ms, loss_val = process_batch(b_tick_start, b_s, b_mids, b_l, b_m, prev_seqs, prev_labels)

                    prev_seqs   = b_s
                    prev_labels = b_l

                    buf_seqs.clear(); buf_mids.clear(); buf_labels.clear(); buf_meta.clear()
                    ...
                    if tick_idx - last_snap >= SNAPSHOT_EVERY:
                        ...
                        snap = {
                            ...
                            "lat_ms": round(lat_ms,3), "loss": loss_val
                        }
                        for h in HORIZONS:
                        ...
                        ls  = f"{loss_val:.4f}" if loss_val is not None else "n/a"
                        print(f"[t={tick_idx:>12,}] TLOB {date_str} | loss={ls} | "
                              f"DA_nz(h1)={da1s} | tps={int(snap['tps']):,}", flush=True)
    ...
    if buf_seqs:
        ...
        b_mids = np.array(buf_mids, dtype=np.float64)
        b_l = np.array(buf_labels, dtype=np.int8)
        process_batch(tick_idx - len(buf_seqs), b_s, b_mids, b_l, buf_meta, prev_seqs, prev_labels)

    elapsed = time.time() - t0
    print(f"\n[DONE TLOB] total_ticks={tick_idx:,}  elapsed={elapsed:.1f}s  tps={tick_idx/elapsed:.0f}", flush=True)
    ...
    summary = {
        "model": "TLOB", "coin": coin, "total_ticks": tick_idx,
    ...
    print(f"\n{'='*85}", flush=True)
    print(f"  FINAL SUMMARY (TLOB)  |  {coin}  |  {tick_idx:,} ticks  |  {elapsed:.0f}s  ({tick_idx/elapsed:.0f} tps)", flush=True)
    ...
    for h in HORIZONS:
        ...
        print(f"  h={h:>4d} | {fmt(hs['da_pa_nz_full']):>9} | {fmt(hs['da_pa_nz_first70']):>8} | "
              ...
              f"{hs.get('n_pa_nz_full', 0):>10,} | {hq._all_total[h]:>10,}", flush=True)
    print(f"\n[OUTPUT TLOB] {RESULTS_DIR}", flush=True)
