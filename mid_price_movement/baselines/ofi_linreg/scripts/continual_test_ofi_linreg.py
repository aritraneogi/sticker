"""
continual_test_ofi_linreg.py  (Classical OFI Linear Regression MPM Prequential Test)
======================================================================================
Strict, leak-free, prequential (test-then-train) continual streaming test
for Mid Price Movement (MPM) prediction using classical Order Flow Imbalance (OFI) Linear Regression.
...
Storage:
  Results: evaluation-test-mpm/results_ofi_linreg/
  Weights: evaluation-test-mpm/results_ofi_linreg/weights/
  Graphs:  evaluation-test-mpm/graphs_ofi_linreg/
"""
...
RESULTS_DIR = SCRIPT_DIR / "results_ofi_linreg"
...
def post_analysis(pred_path: Path, horizons: list[int], split_tick: int) -> dict:
    print("[POST-ANALYSIS OFILinReg] loading predictions.parquet ...")
...
def main():
    ...
    print(f"[CONFIG OFILinReg] coin={coin}  dates={len(all_dates)}  hours={hours[0]}-{hours[-1]}")

    split_tick_est = args.split_tick if args.split_tick else (int(args.max_ticks * SPLIT_RATIO) if args.max_ticks else None)

    # SGDRegressor fitting price impact \Delta M = \beta \cdot OFI_t
    model = SGDRegressor(loss="squared_error", penalty="l2", alpha=args.alpha, learning_rate="optimal")
    ...
    # Find OFI indices in 56-feature vector
    ofi_indices = [i for i, name in enumerate(FEATURE_NAMES) if "ofi" in name or "signed_vol" in name]
    if not ofi_indices:
        ofi_indices = [40, 41, 42, 43] # Default OFI feature indices
    ...
    print("[START OFILinReg] streaming mid price movements ...")

    def process_batch(b_tick_start, b_f, b_mids, b_l, b_m, p_f, p_l):
        nonlocal n_trained

        # Extract OFI sub-features for classical OFI Linear Regression
        ofi_features = b_f[:, ofi_indices]

        t_inf = time.perf_counter()
        if hasattr(model, "coef_"):
            pred_impact = model.predict(ofi_features)
            preds = np.where(pred_impact >= 0, 1, -1)
        else:
        ...
        if p_f is not None and p_l is not None:
            p_ofi = p_f[:, ofi_indices]
            valid_mask = p_l != 0
            if valid_mask.any():
                model.partial_fit(p_ofi[valid_mask], p_l[valid_mask].astype(float))
            ...
            if feat_vec is not None:
                ...
                if len(buf_feats) == BS:
                    ...
                    if tick_idx - last_snap >= SNAPSHOT_EVERY:
                        ...
                        print(f"[t={tick_idx:>12,}] OFILinReg {date_str} | "
    ...
    elapsed = time.time() - t0
    print(f"\n[DONE OFILinReg] total_ticks={tick_idx:,}  elapsed={elapsed:.1f}s  tps={tick_idx/elapsed:.0f}")
    ...
    summary = {
        "model": "OFILinearRegression", "coin": coin, "total_ticks": tick_idx,
    ...
    print(f"\n{'='*85}")
    print(f"  FINAL SUMMARY (OFI Linear Regression)  |  {coin}  |  {tick_idx:,} ticks  |  {elapsed:.0f}s  ({tick_idx/elapsed:.0f} tps)")
    print(f"{'='*85}")
    for h in HORIZONS:
