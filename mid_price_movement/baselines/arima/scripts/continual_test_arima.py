"""
continual_test_arima.py  (Online ARIMA / RLS Auto-Regressive MPM Prequential Test)
===================================================================================
Strict, leak-free, prequential (test-then-train) continual streaming test
for Mid Price Movement (MPM) prediction using Online RLS AR(5) / ARIMA model.
"""
...
RESULTS_DIR = SCRIPT_DIR / "results_arima"
...
def parse_args():
    ...
    p.add_argument("--p-lags",      type=int,   default=5)
...
def post_analysis(pred_path: Path, horizons: list[int], split_tick: int) -> dict:
    print("[POST-ANALYSIS ARIMA] loading predictions.parquet ...")
...
class OnlineARIMA_RLS:
    """Online Recursive Least Squares AR(p) ARIMA model"""
    def __init__(self, p_lags=5, lambda_forget=0.999):
        self.p = p_lags
        self.lam = lambda_forget
        self.w = np.zeros((p_lags, 1), dtype=np.float64)
        self.P = np.eye(p_lags, dtype=np.float64) * 100.0
        self.lags = deque(maxlen=p_lags)
        self.prev_mid = None
    ...
    def update(self, mid_val: float) -> int:
        ...
        delta_mid = mid_val - self.prev_mid
        self.prev_mid = mid_val

        if len(self.lags) < self.p:
            self.lags.append(delta_mid)
            return 1

        x_vec = np.array(list(self.lags), dtype=np.float64).reshape(-1, 1)

        # Predict direction BEFORE RLS parameter update (prequential)
        pred_delta = float((self.w.T @ x_vec)[0, 0])
        pred_sign = 1 if pred_delta >= 0 else -1

        # RLS parameter update
        Px = self.P @ x_vec
        denom = float(self.lam + (x_vec.T @ Px)[0, 0])
        K = Px / denom
        error = delta_mid - pred_delta

        self.w = self.w + K * error
        self.P = (self.P - K @ x_vec.T @ self.P) / self.lam
...
def main():
    ...
    print("[START ARIMA] streaming mid price movements ...")
            ...
            if feat_vec is not None:
                ...
                if len(buf_preds) == BS:
                    ...
                    if tick_idx - last_snap >= SNAPSHOT_EVERY:
                        ...
                        print(f"[t={tick_idx:>12,}] ARIMA {date_str} | "
    ...
    elapsed = time.time() - t0
    print(f"\n[DONE ARIMA] total_ticks={tick_idx:,}  elapsed={elapsed:.1f}s  tps={tick_idx/elapsed:.0f}")
    ...
    with open(wpath, "wb") as f:
        pickle.dump({"w": arima.w, "P": arima.P}, f)
    ...
    summary = {
        "model": "ARIMA_RLS", "coin": coin, "total_ticks": tick_idx,
    ...
    print(f"\n{'='*85}")
    print(f"  FINAL SUMMARY (ARIMA)  |  {coin}  |  {tick_idx:,} ticks  |  {elapsed:.0f}s  ({tick_idx/elapsed:.0f} tps)")
    print(f"{'='*85}")
    for h in HORIZONS:
